...
import { switchMap, map, catchError, skip } from 'rxjs/operators';

import { NbAuthResult } from '../../services/auth-result';
import { NbAuthStrategy } from '../auth-strategy';
import { NbAuthStrategyClass } from '../../auth.options';
import { SmqAuthStrategyOptions, smqStrategyOptions } from './smq-strategy-options';
 ...
 * (which is inside of the `SmqAuthStrategy` class), so you have to replace it with a custom helper function
 * if you need it.
 *
 * ```ts
 ...
 */
@Injectable()
export class SmqAuthStrategy extends NbAuthStrategy {

  protected defaultOptions: SmqAuthStrategyOptions = smqStrategyOptions;

  static setup(options: SmqAuthStrategyOptions): [NbAuthStrategyClass, SmqAuthStrategyOptions] {
    return [SmqAuthStrategy, options];
  }

  constructor(protected http: HttpClient, private route: ActivatedRoute, private gds: GDS) {
    super();
  }
