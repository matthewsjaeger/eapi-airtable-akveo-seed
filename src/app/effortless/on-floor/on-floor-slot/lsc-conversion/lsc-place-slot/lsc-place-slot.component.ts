...
@Component({
  selector: 'ngx-lsc-place-slot',
  templateUrl: './lsc-place-slot.component.html',
  styleUrls: ['./lsc-place-slot.component.scss']
})
export class LscPlaceSlotComponent extends EffortlessComponentBase implements OnInit {
  sid: any;
  slot: any;
  ...
  next() {
    ...
    this.gds.smqSlotRepairAdmin.EditSeals(payload).then(resp => {
      if (!resp.ErrorMessage) {
        ...
        this.router.navigateByUrl('effortless/lsc-checklist/' + self.sid);
      }
    });
