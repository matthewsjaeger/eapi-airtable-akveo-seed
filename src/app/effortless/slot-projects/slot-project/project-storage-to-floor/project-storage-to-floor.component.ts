...
@Component({
  ...
  templateUrl: './project-storage-to-floor.component.html',
  styleUrls: ['./project-storage-to-floor.component.scss']
})
export class ProjectStorageToFloorComponent extends EffortlessComponentBase implements OnInit {
  slots: any = [];
  project: any = {};
  ...
  saveAsCSV() {
    ...
    self.gds.smqSlotRepairAdmin.ScheduleStorageToFloorWrite(payload).then(function (reply) {
  ...
  parseChanges(self, base64Encoded) {
    ...
    self.loading = true;
    self.gds.smqSlotRepairAdmin.ScheduleStorageToFloorRead(payload).then(function (reply) {
      self.loading = false;
      if (reply.ErrorMessage) {
